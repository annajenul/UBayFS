...
In the plot, block constraints are shown with dashed lines, while solid lines represent ordinary (feature-)constraints.

```{r, include = TRUE}
evaluateMultiple(state = model$output$feature_set[1,], model = model)
block_admissibility(state = unlist(model$output$feature_set[1,]), 
                    constraints = model$constraint.params$block_constraints, log = FALSE)
```
