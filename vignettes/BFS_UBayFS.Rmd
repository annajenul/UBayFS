...
Not only are block-wise constraints useful in many cases, but prior weights are also easier to specify at the block level than at the feature level. Thus, we define a help function to build the vector of prior block weights. Features from the same block get assigned the same prior weight. In this example, feature weights in block 1 are set to 0.5, feature weights in block 2 are set to 1, and feature weights in block 3 are set to 2.
```{r, include = TRUE}
...
We observe, that the ``summary()`` prints an additional line for block constraints. All further steps are similar to the UBayFS variant without block constraints. In this example, three features from the third block are selected. Hence, all constraints are fulfilled.
```{r, include = TRUE}
model = train(model)
summary(model)
plot(model)
