...
UBayFS includes to types of expert knowledge: prior feature weights and feature set constraints. 
...
$$f_{\text{Dir}}(\theta;\alpha) = \frac{1}{\text{Be}(\alpha)} \prod\limits_{n=1}^N \theta_n^{\alpha_n-1},$$
where $\text{Be}(.)$ denotes the multivariate Beta function. Generalizations of the Dirichlet distribution [@wong:gdirichlet,@hankin:hyperdirichlet] are implemented in UBayFS as well.
...
Constraints are represented as a linear optimization problem $A\cdot \delta-b\leq 0$, where $A\in\mathbb{R}^{K\times N}$ and $b\in\mathbb{R}^K$. $K$ is defined as the total number of constraints. In general, a feature set $\delta$ is admissible only if $\left(a^{(k)}\right)^T\delta - b^{(k)} \leq 0$, according to the inadmissibility function
